...
if IS_LARGE_DATASET:
    feature = feature.reshape((-1, 7)); target = target.reshape((-1, 1)); adjust_close = adjust_close.reshape((-1, 1)); market_cap = market_cap.reshape((-1, 1))
    X = np.concatenate((feature, adjust_close, market_cap), axis=1)
    idx = np.isnan(X).any(axis=1) | np.isnan(target).any(axis=1)
    X = X[~idx, :].copy(); Y = target[~idx, :].copy()
else:
    id = 2
    feature = feature[id, :, :].reshape((-1, 7))
    target = target[id, :].reshape((-1, 1))
    adjust_close = adjust_close[id, :].reshape((-1, 1))
    market_cap = market_cap[id, :].reshape((-1, 1))
    X = np.concatenate((feature, adjust_close, market_cap), axis=1)
    Y = target.copy()
del data, feature, target, adjust_close, market_cap


#%%
class linear_regression_vanilla:
    def __init__(self, X, Y, X_columns=None, is_normalize=True, test_size_ratio=0.2):
    ...
    def fit(self, cov_type="HC1", is_output=True):
        '''
        params:
            X: 
                - None, diagonose and fit the model
                - feature matrix, shape (n, p) -- predict the target
        '''
    ...
    def nonlinearity(self, smoother_type="polynomial"):
        ...
        R2_hist = np.zeros((self.p, self.p)); R2_hist[:] = np.nan
        ...
        Y_pred = ols.predict(sm.add_constant(self.X_test, prepend=True))
        ...
        for i in range(self.p):
            for j in range(self.p):
                ...
                R2_hist[i, j] = ols.rsquared-self.ols.rsquared
        ...
        plt.subplot(2, 2, 1)
        plt.imshow(R2_hist, cmap='RdBu_r', aspect='auto')
        plt.xticks(range(self.p), self.X_columns, rotation=90)
        plt.yticks(range(self.p), self.X_columns)
        plt.colorbar()
        plt.title(r"$R^2 - R^2_{bench}$")

        plt.subplot(2, 2, 2)
        plt.imshow(AIC_hist, cmap='RdBu_r', aspect='auto')
        plt.xticks(range(self.p), self.X_columns, rotation=90)
        plt.yticks(range(self.p), self.X_columns)
        plt.colorbar()
        ...
        plt.subplot(2, 2, 3)    
        plt.imshow(BIC_hist, cmap='RdBu_r', aspect='auto')
        plt.xticks(range(self.p), self.X_columns, rotation=90)
        plt.yticks(range(self.p), self.X_columns)
        plt.colorbar()
        ...
        plt.subplot(2, 2, 4)
        plt.imshow(CV_error, cmap='RdBu_r', aspect='auto')
        plt.xticks(range(self.p), self.X_columns, rotation=90)
        plt.yticks(range(self.p), self.X_columns)
        plt.colorbar()
    ...
    def outlier(self, threshold="strict"):
        if not hasattr(self, "ols"):
            raise Exception("Fit the model first.")
        ...
        outlier_summary[self.outlier_test["leverage"], 1] = 1
        outlier_summary[self.outlier_test["cooks_distance"], 2] = 1
        outlier_summary[self.outlier_test["p_value"], 3] = 1
        ...
        plt.yticks(range(4), ["stu. res.", "leverage", "cooks dist.", "p-value"])

        plt.subplot(4, 1, 4)
        plt.scatter(range(self.n), np.sum(outlier_summary, axis=1), s=1, color="red")
    ...
    def test_residual_normality_independence(self, lag_max=1):
        if not hasattr(self, "ols"):
            raise Exception("Fit the model first.")
        print("------ Residual normality independent test ------")
        ...
        print("Shapiro-Wilk test statistic: %.4f, p-value: %.4f" % (shapiro_test_statistic, shapiro_p_value))
        ...
        print("Anderson-Darling test statistic: %.4f, p-value: %.4f" % (result.statistic, anderson_p_value))
        ...
        print("Jarque-Bera test statistic: %.4f, p-value: %.4f" % (jarque_bera_test_statistic, jarque_bera_p_value))
        ...
        print("Durbin-Watson test statistic: %.4f, p-value: %.4f" % (durbin_watson_test_statistic, durbin_watson_p_value))
        ...
        print("Breusch-Godfrey test statistic: %.4f, p-value: %.4f" % (breusch_godfrey_test_statistic, breusch_godfrey_p_value))
        ...
        lb_test_statistic, lb_p_value = result[:, 0], result[:, 1]
        print("Ljung-Box test statistic: %.4f, p-value: %.4f" % (lb_test_statistic, lb_p_value))
        ...
        print("Wald-Wolfowitz test statistic: %.4f, p-value: %.4f" % (wald_wolfowitz_test_statistic, wald_wolfowitz_p_value))
    ...
    def feature_selection_all(self):
        ...
        self.feature_selection_forward_stepwise(criterion="AIC (in-sample)")
    ...
    def feature_selection_best_subset(self, criterion="R2 (in-sample)"):
        ...
        for feature_num in tqdm.tqdm(range(1, self.p + 1)):
        ...
        plt.subplot(4, 1, 4)
        print("Criterion: out-of-sample R2 ---")
        for feature_num in np.arange(0, self.p + 1):
    ...
    def feature_selection_forward_stepwise(self, criterion="AIC (in-sample)"):
        ...
        R2_no_feature = 1 - np.sum((self.Y - np.mean(self.Y))**2)/(np.sum((self.Y - np.mean(self.Y))**2))
        ...
        for feature_num in np.arange(1, self.p+1, 1):
            ...
            print("feature_num: %d, best feature: %s, R^2 (in-sample): %.4f, AIC (in-sample): %.4f, BIC (in-sample): %.4f, R^2 (out-of-sample)" % (feature_num, log[feature_num][0][0], log[feature_num][0][1], log[feature_num][0][2], log[feature_num][0][3]))
        ...
        plt.suptitle("Feature selection by forward stepwise selection with criterion criterion: %s" % criterion)
        plt.tight_layout()

    def feature_selection_ridge_lasso(self):
        # Ridge regression
        ...
        for alpha in alpha_list[::-1]:
            coef = log[alpha][0].flatten()
            if len(np.where(np.abs(coef) > beta_threshold)[0]) > feature_num:
                feature_num = len(np.where(np.abs(coef) > beta_threshold)[0])
                self.feature_selection_ridge_summary[feature_num] = [list(np.where(np.abs(coef) > beta_threshold)[0]), log[alpha][2]]
                print("Ridge: feature number: %d, alpha: %.4f, seleced feature index: %s" % (feature_num, alpha, self.feature_selection_ridge_summary[feature_num][0]))
        ...
        for alpha in alpha_list[::-1]:
            ...
            if len(np.where(np.abs(coef) > beta_threshold)[0]) > feature_num:
                feature_num = len(np.where(np.abs(coef) > beta_threshold)[0])
                self.feature_selection_lasso_summary[feature_num] = [list(np.where(np.abs(coef) > beta_threshold)[0]), log[alpha][2]]
                print("Lasso feature number: %d, alpha: %.4f, seleced feature index: %s" % (feature_num, alpha, self.feature_selection_lasso_summary[feature_num][0]))
                critical_alpha.append(alpha)
    ...
    def feature_selection_least_angle_regression(self):
        ...
        for i in range(len(alpha_path)):
            ...
            if len(np.where(np.abs(coef) > beta_threshold)[0]) > feature_num:
                feature_num = len(np.where(np.abs(coef) > beta_threshold)[0])
                self.feature_selection_least_angle_regression_summary[feature_num] = [list(np.where(np.abs(coef) > beta_threshold)[0])]
                print("Least Angle Regression: feature number: %d, alpha: %.4f, seleced feature index: %s" % (feature_num, alpha, self.feature_selection_least_angle_regression_summary[feature_num][0]))
                critical_alpha.append(alpha)
...
#%% 
model = linear_regression_vanilla(X, Y, X_columns=["factor {}".format(i) for i in range(X.shape[1])])
model.visualize_data()
model.fit()
model.nonlinearity()
outlier_result = model.outlier(threshold="strict")
outlier_idx = np.where(np.sum(outlier_result["summary"], axis=1) > 0)[0]


#%%
feature_idx = [0, 1, 2, 4, 6, 7, 8]
X_new = X[~np.isin(np.arange(X.shape[0]), outlier_idx), :][:, feature_idx]
Y_new = Y[~np.isin(np.arange(Y.shape[0]), outlier_idx), :]
model = linear_regression_vanilla(X_new, Y_new, X_columns=["factor {}".format(i) for i in feature_idx])
model.visualize_data()
model.fit()
model.nonlinearity()
model.outlier()
...
class ridge_lasso_regression:
    def __init__(self, X, Y, X_columns=None, is_normalize=True, test_size_ratio=0.2, regularization="ridge"):
        self.X = X; self.Y = Y.reshape(-1, 1)
        self.X_columns = X_columns if X_columns is not None else [f"feature_{i}" for i in range(X.shape[1])]
        self.is_normalize = is_normalize
        self.test_size_ratio = test_size_ratio
...
X_new = X[~np.isin(np.arange(X.shape[0]), outlier_idx), :][:, feature_idx]
Y_new = Y[~np.isin(np.arange(Y.shape[0]), outlier_idx), :]
model = ridge_lasso_regression(X_new, Y_new, X_columns=["factor {}".format(i) for i in feature_idx], regularization="ridge")
model.optimal_alpha()
...
class principal_component_regression:
    ...
    def optimal_factor_number(self, criterion = "R^2 (out-of-sample)", is_plot=True):
        # Eigenvalue spectrum
        U, S, VT = np.linalg.svd(self.X, full_matrices=False, compute_uv=True)
        Z = np.dot(U, np.diag(S)).copy()
        S = np.power(S, 2) / self.n
        ...
        if is_plot:
            plt.figure(figsize=(6, 3))
            plt.plot(np.arange(1, self.p+1, 1), S, "o-")
...
X_new = X[~np.isin(np.arange(X.shape[0]), outlier_idx), :][:, feature_idx]
Y_new = Y[~np.isin(np.arange(Y.shape[0]), outlier_idx), :]
model = principal_component_regression(X_new, Y_new, X_columns=["factor {}".format(i) for i in feature_idx])
...
X_new = X[~np.isin(np.arange(X.shape[0]), outlier_idx), :][:, feature_idx]
Y_new = Y[~np.isin(np.arange(Y.shape[0]), outlier_idx), :]
model = partial_least_square_regression(X_new, Y_new, X_columns=["factor {}".format(i) for i in feature_idx])
model.optimal_factor_number()
